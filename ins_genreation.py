...
#
# def gen_knapsack_ins(capacity=100,largeRatio=0.3,largeLB=0.8,largeUB=1.0,smallLB=0.1,smallUB=0.3,nItems=10):
#
#     largeItemTypes = list(range(int(largeLB*capacity)+1,int(largeUB*capacity)+1))
#     smallItemTypes = list(range(int(smallLB*capacity)+1,int(smallUB*capacity)+1))
#
...
if __name__ == '__main__':
    ...
    insDir = 'data/BPP/nItem20_s300/train/instances'
    nIns = 300
    os.makedirs(insDir,exist_ok=True)
    for i in range(nIns):

        m = gen_bin_packing_ins(capacity=100,largeRatio=0.3,largeLB=0.8,largeUB=1.0,smallLB=0.1,smallUB=0.3,nItems=10)
